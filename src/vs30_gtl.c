...
int gtl_setup(char *file_path) {
  ...
  */
  ...
  gtl.x = roundf((gtl.extent[1] - gtl.extent[0]) / gtl.spacing) + 1;
  gtl.y = roundf((gtl.extent[3] - gtl.extent[2]) / gtl.spacing) + 1;
    
  /* Allocate memory buffer for GTL */
  ncells = gtl.x * gtl.y;
  bufsize = ncells * gtl.dsize;
  gtlbuffer=(void *)malloc(bufsize);
  if (gtlbuffer == NULL) {
    return(1);
  }
